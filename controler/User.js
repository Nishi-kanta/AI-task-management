...
const userRegister = async (req, res) => {
  ...
  try {
   ...
   // Block duplicate Admin or Manager (but allow one of each)
if (["Admin", "Manager"].includes(req.body.role)) {
  const isRoleExist = await User.findOne({ role: req.body.role });
  if (isRoleExist) {
    return res.status(400).json({ message: `${req.body.role} already exists` });
  }
}
